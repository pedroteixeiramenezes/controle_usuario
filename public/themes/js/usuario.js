...
$(document).ready(function() {
    ...
    $(document).on('click', '.edit_usuario', function(e) {
        ...
        $.ajax({
            type: "GET",
            url: "usuario/edit-usuario/" + usuario_id,
            success: function(response) {
                ...
                if (response.status == 404) {
                    ...
                    $('#edit_nome').val(response.usuario.nome);
                    $('#edit_cpf').val(response.usuario.cpf);
                    $('#edit_cep').val(response.usuario.cep);
                    $('#edit_logradouro').val(response.usuario.logradouro);
                    $('#edit_bairro').val(response.usuario.bairro);
                    $('#edit_estado').val(response.usuario.estado);
                    $('#edit_cidade').val(response.usuario.cidade);
                    $('#edit_pais').val(response.usuario.pais);
                    $('#edit_numero').val(response.usuario.numero);
    ...
    $(document).on('click', '.ajaxusuario-update', function(e) {
        ...
        $.ajax({
            type: "PUT",
            url: "usuario/update-usuario/" + usuario_id,
            data: data,
            dataType: "json",
            success: function(response) {
        ...
        swal({
            ...
            .then((willDelete) => {
                if (willDelete) {
                    ...
                    $.ajax({
                        type: "DELETE",
                        url: "usuario/delete-usuario/" + usuario_id,
                        data: data,
                        success: function(response) {
                            location.reload();
    ...
    $(function() {
        $('.toggle-class-servicos').change(function() {
            ...
            $.ajax({
                type: "GET",
                dataType: "json",
                url: 'usuario/modificarStatusPagamento',
                data: {
                    'servicos': servicos,
                    'usuario_id': usuario_id
                },
